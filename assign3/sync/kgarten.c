...
void child_enter(struct thread_info_struct *thr)
{
        ...
        while((thr->kg->vc+1) > thr->kg->vt*thr->kg->ratio) {//c+1 > t * r
                printf("child is waiting, vc = %d\n", thr->kg->vc);
...
void child_exit(struct thread_info_struct *thr)
{
        ...
        --(thr->kg->vc);
/* A child has exited,
 * it has to signal to all other threads 
 ...
 * it has to signal to all other threads
 * to check on their condition again
 */     
        ++(thr->kg->vt);
        pthread_cond_broadcast(&thr->kg->cond);
...
void teacher_exit(struct thread_info_struct *thr)
{
        ...
        while((thr->kg->vt-1)*thr->kg->ratio < thr->kg->vc) { // c > t * r
                pthread_cond_wait(&thr->kg->cond, &thr->kg->mutex);        
        }
        // done adding
        --(thr->kg->vt);
