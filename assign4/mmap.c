...
void parent(pid_t child_pid)
{
         ...
         */
        printf("Parent: PA of heap_private_buf -> %ld\n",get_physical_address((uint64_t)heap_private_buf));
         ...
         */
        printf("Parent: PA of heap_shared_buf -> %ld\n",get_physical_address((uint64_t)heap_shared_buf));
...
int main(void)
{
        ...
        heap_private_buf = mmap(NULL, buffer_size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANON | MAP_PRIVATE, -1, 0);
...
//      show_va_info((uint64_t)heap_private_buf);
         ...
         */
        printf("PA of heap_private_buf :%ld\n",get_physical_address((uint64_t)heap_private_buf));
        ...
        file_shared_buf = mmap(NULL, fileinfo.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        ...
        //close(fd);
        ssize_t n = write(1,file_shared_buf,fileinfo.st_size);
         ...
         */
        heap_shared_buf = mmap(NULL, buffer_size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
